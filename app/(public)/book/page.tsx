...
import { TAX_RATE, SERVICE_CHARGE_RATE, POLICIES } from "@/lib/mock-data";
               ...
               {isCartMode ? (
                 <div className="space-y-6 mb-6">
                   {cartItems.map((item, idx) => {
                     ...
                     return (
                       <div key={idx} className="border-b border-white/10 pb-4 last:border-0 last:pb-0 space-y-4">
                         <div className="flex gap-4">
                           <div className="h-16 w-16 relative bg-neutral-800 shrink-0">
                             {(item.roomImage || item.propertyImage) ? (
                               <Image src={item.roomImage || item.propertyImage} alt={item.roomName} fill className="object-cover" />
                             ) : (
                               <div className="w-full h-full flex items-center justify-center text-neutral-600 text-[8px]">No Image</div>
                             )}
                           </div>
                           <div className="flex-1">
                             <p className="text-[10px] uppercase tracking-widest text-neutral-500">{item.propertyName}</p>
                             <p className="font-medium text-sm">{item.roomName}</p>
                             <p className="text-right text-sm font-medium mt-1">₱{(item.roomPrice * itemNights).toLocaleString()}</p>
                           </div>
                         </div>
                         ...
                         <div className="grid grid-cols-3 gap-2">
                           <div className="space-y-1">
                             <label className="text-[10px] uppercase tracking-widest text-neutral-500">Check-in</label>
                             <Popover>
                               <PopoverTrigger asChild>
                                 <Button variant="outline" className="w-full h-9 text-xs bg-transparent border-white/10 hover:bg-white/5 justify-start px-2">
                                   <CalendarIcon className="w-3 h-3 mr-1 opacity-50 shrink-0" />
                                   <span className="truncate">{format(new Date(item.checkIn), "MMM d")}</span>
                                 </Button>
                               </PopoverTrigger>
                                 ...
                                 <Calendar
                                   ...
                                   onSelect={(newDate) => {
                                     if (newDate) {
                                       ...
                                       updated[idx] = { ...updated[idx], checkIn: newDate };
                           ...
                           <div className="space-y-1">
                             <label className="text-[10px] uppercase tracking-widest text-neutral-500">Check-out</label>
                             <Popover>
                               <PopoverTrigger asChild>
                                 <Button variant="outline" className="w-full h-9 text-xs bg-transparent border-white/10 hover:bg-white/5 justify-start px-2">
                                   <CalendarIcon className="w-3 h-3 mr-1 opacity-50 shrink-0" />
                                   <span className="truncate">{format(new Date(item.checkOut), "MMM d")}</span>
                                 </Button>
                               </PopoverTrigger>
                               <PopoverContent className="w-auto p-0 bg-neutral-900 border-white/10" align="start">
                                 <Calendar
                                   ...
                                   onSelect={(newDate) => {
                                     if (newDate) {
                                       const updated = [...cartItems];
                                       updated[idx] = { ...updated[idx], checkOut: newDate };
                                       setCartItems(updated);
                                     }
                                   }}
                                   disabled={(d) => d <= new Date(item.checkIn)}
                                   className="bg-neutral-900 text-white"
                                 />
                               </PopoverContent>
                             </Popover>
                           ...
                           <div className="space-y-1">
                             ...
                             <select 
                               ...
                               onChange={(e) => {
                                 const updated = [...cartItems];
                                 updated[idx] = { ...updated[idx], guests: parseInt(e.target.value) };
                                 setCartItems(updated);
                               }}
                               className="w-full bg-transparent border border-white/10 text-white h-9 px-2 text-xs rounded-none focus:border-orange-500/50 focus:ring-0 cursor-pointer appearance-none"
                             >
                               {[1, 2, 3, 4, 5, 6].map((num) => (
                                 <option key={num} value={num} className="bg-neutral-900">{num} Guest{num > 1 ? 's' : ''}</option>
                               ))}
                             </select>
                           </div>
                         </div>
                 ...
                 <>
                   <div className="aspect-video relative rounded-none overflow-hidden mb-6 bg-neutral-800">
                      <p className="text-center text-neutral-500 py-8">Please add a room to your cart to continue</p>
                   </div>
                   ...
                   <div className="space-y-4 border-b border-white/10 pb-6 mb-6">
                      <div>
                         ...
                         <p className="font-medium text-lg">Select from properties page</p>
                      </div>
                      <div>
